...
export default function SalaryCalculator() {
    ...
    const calculateNetSalary = () => {
        ...
        const employeeEPF = earnings.reduce((sum, earning) => sum + (earning.epfEtf ? (earning.amount * 0.08) : 0), 0);
        const apit = 3740;
        return basicSalary + grossEarnings - grossDeductions - employeeEPF - apit;
    };

    const calculateCTC = () => {
        const grossEarnings = earnings.reduce((sum, earning) => sum + earning.amount, 0);
        const employerEPF = earnings.reduce((sum, earning) => sum + (earning.epfEtf ? (earning.amount * 0.12) : 0), 0);
        const employerETF = earnings.reduce((sum, earning) => sum + (earning.epfEtf ? (earning.amount * 0.03) : 0), 0);
        return basicSalary + grossEarnings + employerEPF + employerETF;
    };
